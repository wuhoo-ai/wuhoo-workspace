...
#
# 用法:
#   bash ~/.openclaw/scripts/update_stock_data.sh          # 增量更新
#   bash ~/.openclaw/scripts/update_stock_data.sh --full   # 全量重建
#
# 建议 cron: 每个交易日 16:00 (A 股收盘后) 执行
#   0 16 * * 1-5 cd /home/admin/wuhoo-skills/wuhoo-stock-pick && PYTHONUNBUFFERED=1 bash ~/.openclaw/scripts/update_stock_data.sh >> /tmp/stock_data_update.log 2>&1

set -euo pipefail

cd /home/admin/wuhoo-skills/wuhoo-stock-pick
