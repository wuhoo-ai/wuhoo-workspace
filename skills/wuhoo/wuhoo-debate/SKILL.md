...
**v3.1 观察 (2026-05-22)**: Bull 出现**反向收敛** — 12 只美股中 9 只输出 HOLD(0.50)，仅 3 只区分出 BUY(HST 0.75, MET 0.68, CSCO 0.75)。Bull 似乎陷入"安全中立"模式：当因子信号不够极端时，uniformly 输出 HOLD(0.50) 而非基于数据做出差异化判断。这与 v3 修复前的"全 BUY"问题对称：**Bull 从过度乐观转为过度中立**。HOLD(0.50) 是最低信息的输出——既不买入也不卖出，逃避了分析责任。
- **影响**：Trader 在这些股票上无法从 Bull 获取有效信号，只能依赖 Bear（同样以 HOLD(0.55) 为主）
- **根因推测**：prompt 的反偏规则表可能矫枉过正，或场景映射表覆盖不全
- **待修复**：需要增加更多中间场景（如"双动量负 + 中等换手 + 高残差波"→ HOLD 0.45-0.55 NOT 精确 0.50），强制 Bull 在因子数据间产生区分度
