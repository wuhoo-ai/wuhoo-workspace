...
# 加载环境变量
env_file = Path.home() / 'wuhoo-workspace' / '.hermes' / '.env'
if env_file.exists():
...
# 添加 debate 模块路径
debate_path = Path.home() / 'wuhoo-workspace' / 'data' / 'debate'
...
class WorkflowBHandler:
    ...
    def __init__(self, symbol: str, company_name: Optional[str] = None):
        ...
        self.output_dir = Path.home() / 'wuhoo-workspace' / 'data' / 'workflow_b' / f"{safe_symbol}_{self.date}"
