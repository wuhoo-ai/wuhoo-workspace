...
class WorkflowCHandler:
    ...
    def _debate_quick(self, stocks: List[Dict]) -> Dict:
        ...
        for stock in stocks[:5]:
            ...
            risk_approval = "REJECT" if len(risk_factors) >= 2 else ("CONDITIONAL" if len(risk_factors) == 1 else "APPROVE")
            ...
            print(f"  {code} {name}: {recommendation} (置信度{confidence:.0%}), 风控:{risk_approval}")
    ...
    def step5_execute_trades(self, recommendations: Dict) -> Dict:
        ...
        - US: acc_id=18767299, min_qty=1, DAY
        """
        print("\n" + "=" * 60)
        print("Step 5: 执行交易")
        print("=" * 60)
