    ...
    def _check_stop_loss(self, order: Dict, position: Dict) -> Dict:
        ...
        if action == 'BUY':
            ...
            if not stop_loss or stop_loss <= 0:
                return {
                    'passed': False,
                    'message': f'{code} 买入订单未设置止损价'
                }
            # 检查止损幅度是否在允许范围内
            if limit_price > 0:
                stop_loss_pct = (limit_price - stop_loss) / limit_price
                max_stop = self.config['single_stop_loss']
                if stop_loss_pct > max_stop:
                    return {
                        'passed': False,
                        'message': f'{code} 止损幅度过大：{stop_loss_pct*100:.1f}% > {max_stop*100:.0f}% 限制'
                    }

        # 2. 检查当前持仓是否已触发止损
        if code in position.get('positions', {}):
            pos = position['positions'][code]
            cost_basis = pos.get('cost_basis', 0)
            ...
            if cost_basis > 0 and qty > 0:
                ...
                if current_price > 0 and avg_cost > 0:
                    ...
                    if drawdown >= self.config['single_stop_loss']:
                        return {
                            'passed': False,
                            'message': f'{code} 已触发止损：从成本价下跌 {drawdown*100:.1f}% (阈值 {self.config["single_stop_loss"]*100:.0f}%)'
                        }
