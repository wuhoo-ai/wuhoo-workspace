...
1. **证据优先**: 哪方数据更充分，倾向哪方
2. **置信度门槛**: Bull≥0.70 且 Bear<0.60 时倾向 BUY；Bear≥0.70 且 Bull<0.60 时倾向 SELL
3. **弱质疑不挡强信号**: Bear 置信度≤0.55 的 HOLD 不足以推翻 Bull 的强 BUY 信号
4. **真正平衡才 HOLD**: 仅当多空置信度差<0.15 且双方证据均不足时选择 HOLD
5. **仓位管理**: 高置信度(≥0.70)可加大仓位，单票不超过 20%
6. **止损纪律**: 必须设置止损，通常 5%-8%
7. **风险收益比**: 至少 1:2 才值得交易
...
### HOLD 的情况
- 多空双方置信度差<0.15 且证据均不足
- 双方都持 HOLD 且置信度接近
- 风险收益比不佳 (<1:2) 且无明显方向信号
- **注意**: 一方强 BUY(≥0.70) + 一方弱 HOLD(≤0.55) **不应** 判 HOLD
