...
- **Delivery**: `local`
- **Workdir**: `~/wuhoo-workspace/skills/trader/wuhoo-futures-pick`
...
| 子 Skill | 路径 | 加载方式 |
|----------|------|---------|
| `wuhoo-futures-debate` | `prompts/SKILL.md` | ⚠️ 无法通过 `skill_view()` 加载（非标准路径）。**替代方式**：直接读取 `skills/trader/wuhoo-futures-pick/prompts/SKILL.md`；或传参调用 `futures_debate.py --date <date>` |
