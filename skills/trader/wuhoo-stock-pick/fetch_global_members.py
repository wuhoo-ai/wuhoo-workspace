...
def save_members(codes, filepath, metadata_df=None):
    ...
    # 优先使用 metadata_df 保存（包含 code + name + 市值等信息）
    if metadata_df is not None and not metadata_df.empty:
        # 确保包含 code 和 name 列
        save_cols = ['code']
        if 'name' in metadata_df.columns:
            save_cols.append('name')
        if 'ts_code' in metadata_df.columns:
            save_cols.append('ts_code')
        if 'total_market_val' in metadata_df.columns:
            save_cols.append('total_market_val')
        metadata_df[save_cols].to_csv(filepath, index=False)
        print(f"  已保存：{filepath} ({len(metadata_df)} 只，含 name)")
    else:
        pd.DataFrame({'code': codes}).to_csv(filepath, index=False)
        print(f"  已保存：{filepath} ({len(codes)} 只)")
