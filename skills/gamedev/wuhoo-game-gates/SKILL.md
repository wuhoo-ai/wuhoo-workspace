...
## B' 动画试点验收（2026-09-03 起）

- 资产门: `asset_check.py` 检查 10–14 主身大块 / 垂直度 / 接缝遮挡 / 覆盖层。
- 动画门: `frame_diff.py` + `frame_profile.py` 复合判定（loop_gap < 0.3、std ≥ 0.35、停息占比 15–25%、head 区 ≥ 0.8）。
- 导演门: 只做 1 帧试看 + 最终观感裁决；不要求用户逐帧排查装配。
